    ...
    for row = 0, 3 do
        for col = 0, 10 do
            ...
            self.canvas:insertElement({
                ...
                strokeWidth = 1,
                coordinates = {{
                    x = cx + math.cos(math.rad(30)) * hexR,
                    y = cy + math.sin(math.rad(30)) * hexR
                }, {
                    x = cx + math.cos(math.rad(90)) * hexR,
                    y = cy + math.sin(math.rad(90)) * hexR
                }, {
                    x = cx + math.cos(math.rad(150)) * hexR,
                    y = cy + math.sin(math.rad(150)) * hexR
                }}
            })
        end
    end
...
function obj:toggleShow()
    ...
    if self.canvas:isShowing() then
        ...
        local function updateClock()
            ...
            self.canvas[self.dateIndex].text = string.format("%d年%d月%d日 %s", now.year, now.month, now.day,
                ({"周日", "周一", "周二", "周三", "周四", "周五", "周六"})[now.wday])
            ...
            for _, idx in ipairs(self.armorIndices) do
                self.canvas[idx].strokeColor.alpha = breath
            end
            ...
            packetPos = packetPos + 8;
            if packetPos > 190 then
                packetPos = 0
            end
            if packetPos <= 160 then
                self.canvas[self.packetIndex].center.x = 300 + packetPos;
                self.canvas[self.packetIndex].center.y = 190
            else
                local offset = packetPos - 160;
                self.canvas[self.packetIndex].center.x = 460 + offset;
                self.canvas[self.packetIndex].center.y = 190 - (offset * 0.66)
            end
            for _, idx in ipairs(self.visualizerIndices) do
                local h = math.random(2, 12);
                self.canvas[idx].frame.h = h;
                self.canvas[idx].frame.y = 97 - h
            end
            for _, idx in ipairs(self.matrixIndices) do
                if math.random() > 0.8 then
                    self.canvas[idx].fillColor.alpha = 0.6
                else
                    self.canvas[idx].fillColor.alpha = 0.1
                end
            end
        end
        updateClock();
        self.canvas:show();
        self.secTimer = hs.timer.doEvery(0.03, updateClock)
    end
end
